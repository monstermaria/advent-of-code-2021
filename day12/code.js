...
const findAllRoutesRecursively = (currentPoint, connectionTree, route, routes) => {
  ...
  // points described by small letters can not be visited twice, and will be discarded
  if (currentPoint === currentPoint.toLowerCase() && route.indexOf(currentPoint) > -1) {
  ...
  for (let i = 0; i < connectionTree[currentPoint].length; i++) {
    ...
    findAllRoutesRecursively(nextPoint, connectionTree, subRoute, routes);
  }
};
...
const handleStringResult = (result) => {
  ...
  findAllRoutesRecursively('start', connectionsTree, [], routes);
